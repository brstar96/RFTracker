...
# 필수적인 라이브러리를 몇 가지 정리. 라이브러리 설치는 Pycharm File-> Setting 에 들어가서 수행가능
# 데이터를 보고 간단한 연산을 수행 : pandas
# 각종 머신러닝 모델 제공 : scikit-learn
# csv 파일 읽고 쓰기 제공 : cv2
# Python 을 위한 OpenCV 라이브러리 : opencv-python
# 라이브러리 선언부
...
if(Mode=="train"):
    ...
    print(type(TestData))
    scaller2 = StandardScaler().fit(TestData["df"])

    df_scaled_list=[]
    for index in range(0, int(360 / TestDegree)):
        df_scaled = pd.DataFrame(scaller2.transform(df_list[index]), index=df.index.values, columns=df.columns.values)
        df_scaled_list.append(df_scaled)
    f = open("entry.pickle","rb")
    rf2 = pickle.load(f)

    for index in range(0, int(360 / TestDegree)):
        prediction = rf2.predict(df_scaled_list[index])
