...
#     print(tmp[i])
MyData.CreateDataSet(5,"TrainingImages/5degree","5DegreeTraining.csv")
...
boston = MyData.LoadMyData("5DegreeTraining.csv")
features = pd.DataFrame(boston['data'], columns=boston['feature'])  # 입력은 Boston 데이터셋, 칼럼은 보스턴 데이터셋 내의 인덱스를 그대로 가져옴.
print(type(features))
...
df_list = MyData.getTestGrayScale(3,"TestImages/3degree")


print("test grtay list",type(df_list))
# arr = np.array(TestGrayScaleList)
# df = pd.DataFrame(arr.reshape(1, 50*50))
# df_list.append(df)
